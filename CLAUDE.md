...
- `saturation != ""` from an evaluator means the server is overloaded; server-sim skips noise injection. `AnalysisData.IsSaturated()` is the canonical check. See `pkg/evaluator/types.go` for the `SaturationXxx` constants (`"bandwidth"`, `"kv_capacity"`, `"overloaded"`).
  - Metrics are populated with degraded-state values (queue-analysis, BLIS post-sim) or, for the BLIS pre-sim path (DES skipped), with a large load-monotonic *saturated* latency derived from the decode-bandwidth model (`docs/blis-saturated-latency.md`, issue #40) rather than zero. `maxRPS` is populated where computable.
