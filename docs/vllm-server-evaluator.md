...
| `defaultConcurrency` | client-side in-flight cap used when a request omits `maxConcurrency`; `0`/omitted falls back to `evaluator.DefaultMaxConcurrency` (256). A request's `maxConcurrency > 0` always wins. |
